...
function loadDashboardData() {
    ...
    const url = API_URL + '?action=dashboard&filters=' + encodeURIComponent(JSON.stringify(filters));
...
function renderInsights(transactions) {
    ...
    if (topCategory) {
        insights.push(`📌 Kategori pengeluaran terbesar adalah <strong>${topCategory[0]}</strong> dengan total ${formatCurrency(topCategory[1])}.`);
    }

    if (kpi.netCashflow > 0) {
        insights.push(`✅ Cashflow Anda positif sebesar ${formatCurrency(kpi.netCashflow)}.`);
    } else if (kpi.netCashflow < 0) {
        insights.push(`⚠️ Cashflow Anda negatif sebesar ${formatCurrency(Math.abs(kpi.netCashflow))}.`);
    } else {
        insights.push(`⚖️ Pemasukan dan pengeluaran Anda seimbang.`);
    }
...
function renderAccountChart() {
    ...
    if (appState.charts.account) {
        appState.charts.account.destroy();
    }
    ...
    appState.charts.account = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: balances.map(b => b.account),
            datasets: [{
                label: 'Saldo Akun',
...
function calculateCashflowData(transactions) {
    ...
    transactions.forEach(t => {
        ...
        if (!data[dateStr]) {
            data[dateStr] = { income: 0, expense: 0, net: 0 };
        }
...
function renderAccountBalances() {
    ...
    balances.forEach(balance => {
        ...
        card.innerHTML = `
            ...
            <div class="amount">${formatCurrency(balance.balance)}</div>
        `;
...
function renderAnalysisSection() {
    ...
    topExpenses.forEach(item => {
        ...
        element.innerHTML = `
            <span class="analysis-item-name">${item.category}</span>
            <span class="analysis-item-amount">${formatCurrency(item.amount)}</span>
        `;
        topExpensesContainer.appendChild(element);
    });

    if (topExpenses.length === 0) {
        topExpensesContainer.innerHTML = '<div style="padding: 20px; text-align: center; color: var(--text-secondary);">Belum ada data</div>';
    }
    ...
    if (topIncomes.length === 0) {
        topIncomesContainer.innerHTML = '<div style="padding: 20px; text-align: center; color: var(--text-secondary);">Belum ada data</div>';
    }
}
...
function renderBudgetSection() {
    ...
    appState.budgets.forEach(budget => {
        ...
        if (percentage > 100) statusClass = 'critical';
        else if (percentage > 90) statusClass = 'critical';
        else if (percentage > 70) statusClass = 'warning';

        let barClass = 'budget-progress-bar';
        if (statusClass === 'warning') barClass += ' warning';
        if (statusClass === 'critical') barClass += ' critical';
        ...
        card.innerHTML = `
            ...
            <div class="budget-info">
                ...
                <span class="budget-value">${formatCurrency(budget.actual)}</span>
            </div>
            <div class="budget-info">
                <span class="budget-label">Sisa:</span>
                <span class="budget-value">${formatCurrency(budget.remaining)}</span>
            </div>
            <div class="budget-progress">
                <div class="${barClass}" style="width: ${Math.min(percentage, 100)}%"></div>
            </div>
...
function handleTransactionSubmit(e) {
    ...
    if (appState.editingTransactionId) {
        payload.id = appState.editingTransactionId;
